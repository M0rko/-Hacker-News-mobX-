...
const NewsListItem = ({ article }) => {
  ...
  return (
    <div className="flex_wrapper">
      <div>
        <Link className="title" to={`/${article.data.id}`} article={article.data}>
          <h3>{article.data.title}</h3>
        </Link>
        <p className="score">Score: {article.data.score}</p>
        <p className="author">Author: {article.data.by}</p>
        <p className="comments">Comments {article.data.descendants}</p>
        <p className="created_at">
          Created at:{' '}
          {new Date(article.data.time * 1000).toLocaleDateString('en-US', {
            hour: 'numeric',
            minute: 'numeric'
          })}
        </p>
